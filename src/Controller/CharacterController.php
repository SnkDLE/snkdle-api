...
public function guess(Request $request, CharacterRepository $characterRepository): JsonResponse
{
    ...
    if (!$guessName) {
        return new JsonResponse(['error' => 'Nom du personnage manquant'], 400);
    }

    $guessedCharacter = $characterRepository->findOneBy(['name' => $guessName]);

    if (!$guessedCharacter) {
        return new JsonResponse(['error' => 'Personnage non trouvé'], 404);
    }

    // TEMPORAIRE : personnage du jour en dur
    $dailyCharacter = $characterRepository->findOneBy(['name' => 'Mikasa']);

    if (!$dailyCharacter) {
        return new JsonResponse(['error' => 'Personnage du jour non défini'], 500);
    }
    ...
    return $this->json([
        'match' => $guessedCharacter->getId() === $dailyCharacter->getId(),
        'comparison' => $comparison,
        'guessed' => $guessedCharacter->getName(),
    ]);
}
#[Route('/quiz/today', name: 'quiz_today', methods: ['GET'])]
public function quizToday(CharacterRepository $characterRepository): JsonResponse
{
    ...
    if (!$dailyCharacter) {
        return $this->json([
            'status' => 'error',
            'message' => 'Aucun personnage du jour trouvé'
    ...
    return $this->json([
        'status' => 'success',
        'data' => [
            'gender' => $dailyCharacter->getGender(),
            'species' => $dailyCharacter->getSpecies(),
            'age' => $dailyCharacter->getAge(),
            'status' => $dailyCharacter->getStatus(),
        ]
    ]);
}
